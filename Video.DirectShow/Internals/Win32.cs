...
namespace AForge.Video.DirectShow.Internals
{
    ...
    internal static class Win32
    {
        ...
        /// <param name="dst">Destination pointer.</param>
        /// <param name="src">Source pointer.</param>
        /// <param name="length">Memory block's length to copy.</param>
        /// 
        /// <returns>Return's the value of <b>dst</b> - pointer to destination.</returns>
        ///
        [DllImport("ntdll.dll", EntryPoint = "memcpy", CallingConvention = CallingConvention.Cdecl)]
        private static extern int memcpyx86(
            int dst,
            int src,
            int length);

        [DllImport("ntdll.dll", EntryPoint = "memcpy", CallingConvention = CallingConvention.Cdecl)]
        private unsafe static extern byte* memcpyx6486(
            byte* dst,
            byte* src,
            int length);

        [DllImport("msvcrt.dll", EntryPoint = "memcpy", CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr memcpyx64(
            IntPtr dst,
            IntPtr src,
            UIntPtr length);

        public static int MemCopy(int dst, int src, int length)
        {
            if (Environment.Is64BitProcess)
                return memcpyx64(new IntPtr(dst), new IntPtr(src), new UIntPtr((uint)length)).ToInt32();
            else
            {
                if (Environment.Is64BitOperatingSystem)
                {
                    unsafe
                    {
                        memcpyx6486((byte*)dst, (byte*)src, length);
                    }
                    return dst;
                }
                else
                    return memcpyx86(dst, src, length);
            }
        }
